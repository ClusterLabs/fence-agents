...
def replace_api_uri(options, http_request):
  ...
  if "FENCE_GCE_URI_REPLACEMENTS" in os.environ:
    ...
    try:
      ...
      if isinstance(uri_replacements_json, list):
        ...
        logging.warning(
            "FENCE_GCE_URI_REPLACEMENTS exists, but is not a JSON List")
    except ValueError:
  ...
  for uri_replacement in uri_replacements:
    ...
    if "matchlength" not in uri_replacement or "match" not in uri_replacement or "replace" not in uri_replacement:
      logging.warning(
          "FENCE_GCE_URI_REPLACEMENTS missing matchlength, match, or replace in %s",
          uri_replacement)
      continue
    match = re.match(uri_replacement["match"], http_request.uri)
    if match is None or len(match.groups()) != uri_replacement["matchlength"]:
      continue
    replaced_uri = re.sub(uri_replacement["match"], uri_replacement["replace"],
                          http_request.uri)
    ...
    if match is None or match.start() != 0:
      ...
      continue
    logging.debug("Replacing googleapis uri %s with %s", http_request.uri,
                  replaced_uri)
...
def retry_api_execute(options, http_request):
  ...
  while retry <= retries:
    ...
    try:
      ...
      current_err = err
      logging.warning(
          "Could not execute api call to: %s, retry: %s, "
...
def get_nodes_list(conn, options):
  ...
  if "--zone" not in options:
    fail_usage(
        "Failed: get_nodes_list: Please specify the --zone in the command")
...
def check_for_existing_operation(conn, options, instance, zone, operation_type):
  ...
  result = retry_api_execute(
      options,
      conn.zoneOperations().list(project=options["--project"], zone=zone))
...
def set_power_status(conn, options):
  ...
  for i, instance in enumerate(options["--plug"].split(";")):
    # If zone is not listed for an entry we attempt to get it automatically
    zone = zones[i] if i < len(zones) else get_zone(conn, options, instance)
    set_instance_power_status(conn, options, instance, zone,
                              options["--action"])
...
def set_instance_power_status(conn, options, instance, zone, action):
  ...
  try:
    if action == "off":
      ...
      operation = check_for_existing_operation(conn, options, instance, zone,
                                               "stop")
      if not operation:
...
def power_cycle_instance(conn, options, instance, zone):
  ...
  try:
    ...
    logging.info(
        "Reset command completed, waiting for the operation to complete")
    wait_for_operation(conn, options, zone, operation)
...
def define_new_opts():
  ...
  all_opt["retries"] = {
      ...
      "longopt": "retries",
      "help":
          "--retries=[retries]            Number of retries on failure for API"
          " calls",
  ...
  all_opt["serviceaccount"] = {
      ...
      "longopt":
          "serviceaccount",
      "help":
          "--serviceaccount=[filename]    Service account json file location "
          "e.g. serviceaccount=/somedir/service_account.json",
      "shortdesc":
  ...
  all_opt["proxyhost"] = {
      "getopt": ":",
      "longopt": "proxyhost",
      "help": "--proxyhost=[proxy_host]       The proxy host to use, if one is "
              "needed to access the internet (Example: 10.122.0.33)",
      "shortdesc":
          "If a proxy is used for internet access, the proxy host should be "
          "specified.",
      "required": "0",
      "order": 10
  }
  all_opt["proxyport"] = {
      ...
      "type": "integer",
      "longopt": "proxyport",
      "help": "--proxyport=[proxy_port]       The proxy port to use, if one is "
              "needed to access the internet (Example: 3127)",
      "shortdesc":
          "If a proxy is used for internet access, the proxy port should be "
          "specified.",
      "required": "0",
      "order": 11
  }
...
def main():
  ...
  all_opt["method"]["help"] = (
      "-m, --method=[method]          Method to fence (onoff|cycle) "
      "(Default: cycle)")
  ...
  docs["longdesc"] = \
               ("fence_gce is an I/O Fencing agent for GCE (Google Cloud "
                "Engine). It uses the googleapiclient library to connect to "
                "GCE.\ngoogleapiclient can be configured with Google SDK CLI or"
                " by executing 'gcloud auth application-default login'.\nFor "
                "instructions see: "
  ...
  if options.get("--stackdriver-logging") is not None and options.get("--plug"):
    try:
      ...
      logging.exception("Couldn't import google.cloud.logging, "
